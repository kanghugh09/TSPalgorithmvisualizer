...
function setup() {
	...
	two_opt_inversion_button.position(width/2 + 500, height/2 + 175);
	...
	slider.position(width/2 - 600, height/2 + 170);


	shufflePoints(slider.value());
	
	shuffleButton = createButton("Shuffle Cities")
	shuffleButton.position(width/2- 450, height/2 + 170);
	...
	nearest_neighbor_button.position(width/2 + 500, height/2 + 85);
...
function draw() {
	...
	textSize(20);
	fill(255);
	text("Number of cities = " + slider.value(), width/2 - 600, height/2 + 165);

	textSize(18);
	text("Two Opt Delay = " + speedSlider.value(), width/2 + 325, height/2 + 170);


	textSize(64);
	var s = pathCost(points);
	fill(255);
	text("Current Distance = " + s, 150, height/2 + 290);

	text("Record Distance = " + globalRecordDistance, 150, height/2 + 360);

	textSize(40);
	text("Traveling Salesman Problem Algorithm Visualizer", 250, 60);
}
